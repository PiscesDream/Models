...
from numpy import asarray, random, inf, mean, zeros, sqrt, array, ones
import time
...
class ann(object):
    ...
    def __init__(self, n_in, n_out, h_layers = [10], lmbd1 = 0, lmbd2 = 0.001, learning_rate = 0.01, n_epochs=5000):
        self.x = T.matrix('x')
        self.y = T.matrix('y')
...
#        self.predict = theano.function(inputs = [seflx], outputs = ann_str.predict(x))
    ...
    def fit(self, datasets, batch_size = 20):
        ...
        print self.ann_str
#        print theano.function(inputs=[], outputs=self.y, givens={self.y:train_set_y})()
#        print theano.function(inputs=[],outputs=T.sgn(self.ann_str.predict().T), givens={self.x:train_set_x})()[0]
...
if __name__ == '__main__':
    ...
    print 'loading...'
    train_x, train_y, train_fn, test_x, test_y, test_fn = \
        cPickle.load(open('../../Data/cifar-10/cifar-10-batches-py/full.dat','rb')) 
    train_x = train_x.reshape(train_x.shape[0], -1)
    test_x = test_x.reshape(test_x.shape[0], -1)
    train_y = boarden(10, array(train_y))
    test_y = boarden(10, array(test_y))
    l = train_x.shape[0]
...
#    Image.fromarray(train_x[0].reshape(-1, size)).show()

    #classifier = ann(train_x.shape[1], 10, [1000, 500, 200], n_epochs = inf)
    classifier = ann(train_x.shape[1], 10, [100, 50, 20], n_epochs = inf)
